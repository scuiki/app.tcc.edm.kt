# IProblemKnowledgeComponentRepository sobre SQLite; roda na IUnitOfWork de quem chama.
...
def _to_entity(row: sqlite3.Row) -> ProblemKnowledgeComponent:
    return ProblemKnowledgeComponent(
        id=row["id"],
...
class SqliteProblemKnowledgeComponentRepository:
    def __init__(self, conn: sqlite3.Connection) -> None:
        self._conn = conn

    def add(self, binding: ProblemKnowledgeComponent) -> int:
        cur = self._conn.execute(
            "INSERT INTO problem_kc (assignment_id, kc_id, problem_id) VALUES (?, ?, ?);",
