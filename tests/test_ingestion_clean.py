...
Herméticos e CPU-only: constroem o cru sobre `a439_mini_progsnap` / `ingest_orphan_df` (Pitfall 2 —
...
def test_no_dedup_warning_when_no_compile_plain(a439_mini_progsnap):
    ...
    df, items = clean_event_stream(a439_mini_progsnap, _A439_CODE_STATES)
    assert set(df["event_type"].unique()).issubset(KEPT_EVENTS)
    assert [i for i in items if i.check == "dedup_compile"] == []


def test_is_correct_binarization_rule(a439_mini_progsnap):
    # is_correct = (Run.Program & Score==1.0); Compile.Error é sempre incorreto (D-12).
    df, _ = clean_event_stream(a439_mini_progsnap, _A439_CODE_STATES)
